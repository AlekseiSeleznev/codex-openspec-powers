...
#### Scenario: Specific template-owned files override merge-review directories
- **WHEN** a file path is listed in `templateOwnedPaths` and its parent
  directory is listed in `mergeReviewPaths`
- **THEN** Codex treats the specific file as template-owned while leaving other
  files in the merge-review directory as merge-reviewed or user-owned
...
- **THEN** `INSTALL_CODEX_TEMPLATE.md` is listed as a template-owned path,
  `README.md` and `AGENTS.md` are listed as merge-review paths, and these
  documentation paths can be checked without overwriting existing project
...
- **THEN** it checks manifest-listed documentation snippets for
  `INSTALL_CODEX_TEMPLATE.md`, `README.md`, and `AGENTS.md` so missing Claude
  review guidance, install guidance, or agent guardrails are reported
