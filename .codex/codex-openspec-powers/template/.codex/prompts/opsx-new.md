---
description: Start a new OpenSpec change from Codex
---

Start a new OpenSpec change.

Input may be a kebab-case change name or a short description. If unclear, ask
one focused question before creating anything.

Steps:

1. Run `openspec new change "<name>"`, omitting `--schema` unless the user
   explicitly requested a schema.
2. Run `openspec status --change "<name>" --json`.
3. Show the first ready artifact instructions with
   `openspec instructions <artifact-id> --change "<name>" --json`.
4. Stop before creating artifacts.
...
1. Вызвать `openspec-git-discipline` - use when starting a lifecycle where
   branches, commits, or merge timing can matter.
2. Использовать `superpowers:brainstorming` - use only when the request is
   vague, high-impact, or has multiple plausible directions.
